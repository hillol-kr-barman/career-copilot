...
import {
  StackingContainer,
  StackingCard,
} from "../components/StackingContainer";
...
const parseSections = (rawText: string): ResumeReportSection[] => {
  ...
  if (parsedKeys.length > 0) {
    return tabNames.map((tab) => ({
      ...
      content:
        ...
        "Information not generated for this version context.",
    }));
  }
  ...
  return tabNames.map((tab, i) => {
    ...
    return {
      ...
      title: tab,
      content:
        "Please write specific prompts or configurations to request structured sections.",
    };
  });
};
...
const renderHiglightedMMR = (text: string) => {
  ...
  const mmrInner = text
    .substring(mmrStartIdx + "[[MMR_START]]".length, mmrEndIdx)
    .trim();
  const after = text.substring(mmrEndIdx + "[[MMR_END]]".length);

  // Filter headers if any
  const formattedMMR = mmrInner
    .replace(/^(Mandatory Minimum Requirements:?\s*)/i, "")
    .trim();
...
export default function LandingPage({
  ...
  const [aiDetectResult, setAiDetectResult] = useState<{
    ...
    engine: string;
    sentences?: Array<{ sentence: string; fakePercentage?: number }>;
  } | null>(null);
...
- Strong system architectural design patterns understanding`
  ...
  // Outputs - Resume Mode
  const [resumeReport, setResumeReport] = useState<ResumeReportData | null>(
    null
  );
  const [isAnalyzingResume, setIsAnalyzingResume] = useState<boolean>(false);
  const [resumeError, setResumeError] = useState<string>("");
  const [activeResumeReportTab, setActiveResumeReportTab] =
    useState<string>("JD Review");
  ...
  const handleAnalyzeResume = async () => {
    ...
    if (!resumeJd.trim()) {
      setResumeError(
        "Please supply a valid Job Description before running audit."
      );
      return;
    }
    if (!resumeText.trim()) {
      setResumeError(
        "Please upload a resume file or enter raw candidate text in Card #2."
      );
      return;
    }
    ...
    try {
      ...
      setResumeError(
        err?.message || "Critical error connecting with server model."
      );
    } finally {
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3 mb-5 pb-4 border-b border-[rgba(255,255,255,0.07)]">
          ...
          <div className="flex-1">
            <h3 className="text-sm font-semibold text-[#eef0f3] tracking-tight flex items-center gap-2">
              ...
              <span className="text-[10px] bg-[rgba(0,212,220,0.12)] text-[#00d4dc] font-mono font-semibold px-2 py-0.5 rounded-[4px] uppercase tracking-wider">
                Zero-Shot
              </span>
            </h3>
            <p className="text-xs text-[#6b7685] mt-0.5">
              Upload or paste document text to detect AI-generated content rate
              before audit.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="flex flex-col gap-3">
            ...
            <textarea
              value={aiDetectText}
              onChange={(e) => {
                setAiDetectText(e.target.value);
                setAiDetectResult(null);
              }}
              rows={4}
            ...
            {!aiDetectResult && !isDetecting && (
              <div className="flex flex-col items-center gap-3 text-center">
                ...
                <p className="text-xs text-[#6b7685] font-sans max-w-[180px] leading-relaxed">
                  Upload or paste text and click{" "}
                  <b className="text-[#9aa3b0]">Check AI Rate</b> to scan.
                </p>
              </div>
            )}
              ...
              (() => {
                ...
                return (
                  <div className="w-full flex flex-col items-center gap-4">
                    {/* Circular gauge */}
                    <div
                      className={`relative w-28 h-28 rounded-full border-4 ${borderColor} flex items-center justify-center bg-[#161a1e]`}
                    >
                      <div className="flex flex-col items-center">
                        <span
                          className={`text-3xl font-extrabold font-mono tracking-tight ${color}`}
                        >
                          {pct}%
                        </span>
                        <span className="text-[9px] font-bold text-[#6b7685] uppercase tracking-wider mt-0.5">
                          AI Rate
                        </span>
                      </div>
                    </div>
      ...
      <StackingContainer>
        ...
        <StackingCard id="jd" title="Job Description">
          <div className="flex flex-col gap-2">
            <label
              htmlFor="resume_jd_editor"
              className="text-[10px] md:text-xs font-semibold text-[#6b7685] uppercase tracking-wider"
            >
            ...
            <div className="flex justify-between items-center text-[10px] text-[#6b7685] font-mono mt-1">
              <span>
                WORD COUNT:{" "}
                {resumeJd.trim().split(/\s+/).filter(Boolean).length}
              </span>
        ...
        <StackingCard id="uploads" title="Document Upload & Extraction">
        ...
        <StackingCard id="notes" title="Prompt Orchestration Notes">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              <label className="text-[10px] md:text-xs font-semibold text-[#6b7685] uppercase tracking-wider">
                Special context context (e.g. "Highlight leadership
                credentials", "Emphasize consulting gaps")
              </label>
              <textarea
                value={promptNotes}
      ...
      {resumeReport && (
        <section
          id="results-panel"
          className="bg-[#161a1e] border border-[rgba(255,255,255,0.07)] rounded-[10px] p-6 md:p-8  transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] flex flex-col gap-6 scroll-mt-24 text-[#eef0f3]"
        >
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[rgba(255,255,255,0.07)] pb-4">
            <div>
              ...
              <p className="text-xs text-[#6b7685] mt-1">
                Report outputs parsed into optimized tabs. Model used:{" "}
                <span className="font-mono text-[11px] font-bold text-[#00d4dc] bg-[rgba(0,212,220,0.08)] px-2 py-0.5 rounded border border-[rgba(0,212,220,0.2)]">
                  {resumeReport.modelUsed}
                </span>
              </p>
            </div>
            <button
              onClick={() =>
                handleDownload("resume-review-report.txt", resumeReport.body)
              }
          ...
          <div className="bg-[#1c2128] rounded-[8px] p-5 md:p-6 border border-[rgba(255,255,255,0.07)] min-h-[250px] transition-all">
            ...
            {(() => {
              const activeSec = resumeReport.tabSections.find(
                (s) => s.tabName === activeResumeReportTab
              );
