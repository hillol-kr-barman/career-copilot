...
export const StackingCard: React.FC<StackingCardProps> = ({
  ...
  const lightBgMap: Record<string, string> = {
    jd: "bg-white/90 backdrop-blur-2xl border-white/60 shadow-lg ring-1 ring-slate-200/50 hover:shadow-xl hover:border-blue-400/30",
    uploads: "bg-white/90 backdrop-blur-2xl border-white/60 shadow-lg ring-1 ring-slate-200/50 hover:shadow-xl hover:border-blue-400/30",
    notes: "bg-white/90 backdrop-blur-2xl border-white/60 shadow-lg ring-1 ring-slate-200/50 hover:shadow-xl hover:border-blue-400/30",
    scoring: "bg-white/90 backdrop-blur-2xl border-white/60 shadow-lg ring-1 ring-slate-200/50 hover:shadow-xl hover:border-blue-400/30",
    metric: "bg-white/90 backdrop-blur-2xl border-white/60 shadow-lg ring-1 ring-slate-200/50 hover:shadow-xl hover:border-blue-400/30",
    report: "bg-white/90 backdrop-blur-2xl border-white/60 shadow-lg ring-1 ring-slate-200/50 hover:shadow-xl hover:border-blue-400/30",
  };

  const currentBg = lightBgMap[id] || "bg-white/90 backdrop-blur-2xl border-white/60 shadow-lg ring-1 ring-slate-200/50 hover:shadow-xl hover:border-blue-400/30";

  return (
    <article
      data-stack-card={id}
      className={`sticky w-full rounded-[1.8rem] ${currentBg} border p-5 md:p-7 transform-gpu hover:scale-[1.012] transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] select-text flex flex-col gap-4 md:gap-5 mb-8 last:mb-0`}
      style={{
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 pb-3">
        <div className="flex items-center gap-2.5">
          <span
            className="px-2.5 py-0.5 text-[9px] md:text-xs font-semibold tracking-wider uppercase rounded-full shadow-xs border"
            style={{
        ...
        <div className="text-[9px] md:text-xs font-mono text-slate-400">
          MODULE ID • {id.toUpperCase()}
        </div>
      </div>
      <div className="flex flex-col gap-4 text-slate-600 font-sans text-sm md:text-base leading-relaxed max-h-[calc(100vh-210px)] overflow-y-auto pr-1 select-text scrollbar-thin">
        {children}
      </div>
    </article>
  );
